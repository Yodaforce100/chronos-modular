...
export default function SVNavbar({ onCTAClick }) {
  ...
  return (
    ...
    >
      ...
      <div className="max-w-5xl mx-auto px-6 h-auto md:h-20 py-3 md:py-0 grid grid-cols-3 items-center">
        {/* Left: nav links */}
        <div className="hidden md:flex items-center gap-6">
          {links.map((link) => (
            <a
              key={link.label}
              href={link.href}
              className="font-sans text-sm font-medium text-[#5E8DBF] hover:text-[#5E8DBF]/80 transition-colors duration-300"
            >
              {link.label}
            </a>
          ))}
        ...
        <div className="flex justify-center items-center min-w-0">
          <a href="#" aria-label="Shepherd Verses home" className="flex items-center">
            <img
            ...
            <div className="px-6 py-6 flex flex-col gap-4">
              {links.map((link) => (
                <a
                  ...
                  className="font-sans text-base text-[#001c5f] py-2"
                >
                  {link.label}
                </a>
              ))}
